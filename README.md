...
### Sétimo projeto do curso de Desenvolvimento Full Stack Web da Trybe.

## Objetivos a serem alcançados no decorrer da construção do projeto:

- Completar as funções e testes unitários de forma a satisfazer os requisitos listados na próxima seção.

Na pasta raíz do projeto, temos a pasta `src` e a pasta `test`. A pasta `src` é composta pelo arquivo `zoo.js`, que contém as funções a serem implementadas, e o arquivo `data.js`, que armazena os dados que serão utilizados. Já a pasta `test`, contém os testes unitários correspondentes a cada função do arquivo `zoo.js`.
...
- Produzindo código legível, conciso e expressivo utilizando as novas funcionalidades do ES6;

- Utilizando as _Higher Order Functions_ para manipular e criar arrays;

- Escolhendo a _Higher Order Function_ mais adequada para a obtenção de um resultado esperado;

- Aprendendo a usar de forma conjunta as _Higher Order Functions_;

- Interpretando testes unitários e produzindo soluções que atendam a eles
